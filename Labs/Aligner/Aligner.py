...
def compute_backpointers(s0, s1):
    ...
    for i in range(m):
        for j in range(n):
            if i == 0:
                ...
                d[i,j] = min( d[i-1,j]+1, (d[i-1,j-1] + subst_cost(s0[i-1], s1[j-1])),d[i,j-1]+1 )
    ...
    for i in range(len(s0),0,-1):
        for j in range(len(s1),0,-1):
            if (i>0 and j>0 and d[i-1][j-1] <= d[i-1][j] and d[i-1][j-1] <= d[i][j-1]):
                    backptr[i][j][0] = i-1
                    backptr[i][j][1] = j-1
            elif (i>0 and d[i-1][j] <= d[i-1][j-1] and d[i-1][j] <= d[i][j-1]):
                backptr[i][j][0] = i-1
                backptr[i][j][1] = j
            else:
                backptr[i][j][0] = i
...
def align(s0, s1, backptr):
    ...
    c_r = len(s0)
    c_c = len(s1)
    while ( c_r>0 or c_c>0 ):
        n_r = c_r
        n_c = c_c
        if ( backptr[c_r][c_c][0] == c_r-1 ):
            b1 = str_join(b1,s0[c_r-1])
            n_r = c_r-1
        else:
            b1 = str_join(b1,' ')
        if ( backptr[c_r][c_c][1] == c_c-1 ): 
            b2 = str_join(b2,s1[c_c-1])
            n_c = c_c-1
        else:
            b2 = str_join(b2,' ')
        c_r = n_r
