...
class BigramTrainer(object):
    ...
    def process_token(self, token):
        ...
        if self.acc != []:
            self.bigram_count[self.index[self.acc[-1]]][self.index[token]] += 1
    ...
    def stats(self):
        ...
        for ke,va in self.unigram_count.items():
            rows_to_print.append(str(ke)+' '+self.word[ke]+' '+str(va))

        for k1,v in self.bigram_count.items():
            for k2,value in v.items():
                rows_to_print.append(str(k1)+' '+str(k2)+' '+'{0:.15f}'.format(math.log(value/self.unigram_count[k1])))
        
        rows_to_print.append(str(self.last_index))
